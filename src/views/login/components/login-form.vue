...
export default {
  ...
  setup () {
    ...
    // vee-validate 校验基本步骤
    // 1.导入Form Field 组件将form和input进行替换，需要加上name属性，用来指定校验规则函数
    // 2.Field 需要进行数据绑定,字段名称最好和后台接口需要一致
    // 3.定义Field的name属性指定的校验规则函数，Form的validation-schema接收定义好的校验规则
    // 4.自定义组件需要校验必须先支持v-model，然后Filed使用as指定组件名称
    const Myschema = {
